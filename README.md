# Populus — A Populous: The Beginning World Simulator
...
Recreate the complete game experience of Populous: The Beginning — the spherical planetoid, real-time terrain manipulation, autonomous tribal settlements, spell system, and unit AI — **plus extend it** with autonomous fauna/flora ecosystems, weather, day/night cycles, and biome-driven world simulation. All visuals are procedural placeholders (colored geometry, no sprites).

**World-First Philosophy**: The entire planet must live, breathe, and simulate autonomously before any player interaction is added. Tribes settle, animals hunt, trees grow, weather changes — all without a human player. The player is the *last* thing we build.

The architecture is designed so that **any new behavior or system can be added by dropping in one Component file and one System file** — no rewiring, no deep inheritance chains.
...
- **First 3D entry**: Used a custom-built 3D engine with a **spherical planetoid** that could be rotated 360°.
- **"Smart" Followers**: Villagers (Braves) autonomously build huts, harvest wood, and populate — the player only needs to direct military actions and spell casting.
...
- **No Formal Resources**: Only **wood** (for building) and **mana** (generated by housed followers, for spells). No gold, no food — just population management.
...
|---|---|---|
| **Topology** | Flat torus projected to sphere | Flat torus grid → sphere projection via vertex shader |
| **Grid** | Square tiles, uniform size | 128×128 or 256×256 tile grid, wrapping on both axes |
| **Height** | Per-vertex heightmap, discrete levels | Per-vertex heightmap with smooth interpolation |
| **Manipulation** | Raise, Lower, Flatten, Landbridge, Erode, Volcano, Earthquake | Same — each is an ECS System operating on HeightmapComponent |
| **Textures** | Altitude-based blending with masks | Procedural shader: altitude + moisture + temperature → biome texture |
...
| **Warrior Training Hut** | Converts Braves → Warriors | Wood + flat terrain |
| **Temple of Fire** | Converts Braves → Firewarriors | Wood + flat terrain |
| **Temple** | Converts Braves → Preachers | Wood + flat terrain |
| **Spy Training Hut** | Converts Braves → Spies | Wood + flat terrain |
...
- **One-Shot Spells**: Obtained from Stone Heads, Obelisks, Gargoyles — used once and gone.
- **Permanent Spells**: Learned at Vaults of Knowledge — rechargeable with mana.
...
- **Storms**: Lightning strikes, tornado spawns, heavy rain — can damage structures.
...
1. **Massive Entity Count**: Thousands of followers, animals, trees, tiles — ECS handles this with cache-friendly data iteration.
2. **Composable Behaviors**: A deer is `[com_position, com_health, com_hunger, com_species, com_prey, com_herd]`. A wolf swaps `com_prey` → `com_predator`. No deep inheritance trees.
3. **Drag-and-Drop Systems**: Want a "Disease System"? Create `com_disease.gd` and `sys_disease.gd` — it automatically affects any entity with `com_health + com_disease`. Zero changes to existing code.
...
├── DOCS_SYSTEMS.md                    # Detailed system implementation specs
├── PLAN.md                            # 14-phase implementation roadmap
│
├── ecs/                               # ─── ECS CORE (4 files, never changes) ───
...
│
├── components/                        # ─── ALL COMPONENTS (pure data, flat, com_ prefix) ───
...
│   ├── com_species.gd                 # name, diet, preferred_biomes (fauna)
│   ├── com_growth.gd                  # stage, rate, age, max_age (flora)
│   ├── com_flammable.gd               # flammability, is_burning, burn_timer
│   └── ...                            # 40+ more (see PLAN.md for full list)
│
├── systems/                           # ─── ALL SYSTEMS (pure logic, flat, sys_ prefix) ───
...
│   ├── sys_time.gd                    # advance game clock, day/night, seasons
...
│   └── ...                            # 30+ more (see PLAN.md for full list)
│
├── data/                              # ─── STATIC DEFINITIONS (readonly config, def_ prefix) ───
...
│
├── planet/                            # ─── PLANET GEOMETRY (pure math/rendering) ───
...
│
├── generation/                        # ─── PROCEDURAL GENERATION (one-shot, gen_ prefix) ───
...
Matches the original game exactly. All game logic operates on a flat wrapping grid (128×128). Only the rendering projects to a sphere. This massively simplifies pathfinding, terrain manipulation, and spatial queries.
...
No nested subfolders inside `components/` or `systems/`. The `com_` / `sys_` / `def_` / `gen_` prefixes group things naturally. When you have 60 components, you `Ctrl+P` and type `com_health` — done.
...
| Part | Phases | What |
|---|---|---|
| **A — Foundation** | 0-5 | ECS core, torus grid, sphere mesh, terrain gen, biomes, debug camera |
| **B — Nature Lives** | 6-9 | Time/day-night/seasons, weather, flora ecosystem, fauna ecosystem |
| **C — Civilization Lives** | 10-12 | Autonomous tribes, buildings/mana/population, spells/combat |
| **D — Player Enters** | 13 | Player tribe, input, UI, save/load |
...
- [Populous Wiki](https://wiki.popre.net/) — Community wiki with detailed game mechanics (mana, hut breeding, spells)
- [Red Blob Games — Procedural Planet Generation](https://www.redblobgames.com/x/1843-planet-generation/) — Spherical terrain generation techniques
- [GameDev StackExchange — Populous Geometry](https://gamedev.stackexchange.com/questions/16317/geometry-system-of-populous-the-beginning) — Torus-to-sphere analysis
- [ECS Wikipedia](https://en.wikipedia.org/wiki/Entity_component_system) — ECS architecture overview
