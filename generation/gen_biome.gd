...
func generate(grid: TorusGrid, temperature_map: PackedFloat32Array, moisture_map: PackedFloat32Array) -> void:
	_generate_temperature(grid, temperature_map)
	_generate_moisture(grid, moisture_map)
	print("Generated biome data: temperature + moisture maps.")


func _generate_temperature(grid: TorusGrid, temp_map: PackedFloat32Array) -> void:
	...
	for y in range(h):
		var latitude_factor: float = 1.0 - abs((float(y) / float(h)) * 2.0 - 1.0)
		for x in range(w):
			var base_temp: float = latitude_factor
			var altitude := grid.get_height(x, y)
			var altitude_cooling := maxf(altitude, 0.0) * 0.4
			var noise_var := noise.get_noise_2d(float(x), float(y)) * 0.15
			var temp := clampf(base_temp - altitude_cooling + noise_var, 0.0, 1.0)
			temp_map[y * w + x] = temp


func _generate_moisture(grid: TorusGrid, moist_map: PackedFloat32Array) -> void:
	var w := grid.width
	var h := grid.height
	...
	for y in range(h):
		for x in range(w):
			...
			var noise_var := noise.get_noise_2d(float(x), float(y)) * 0.2 + 0.1
			var wind_bias := sin(float(x) / float(w) * TAU) * 0.08
