...
static var _tables_initialized := false
...
static func generate_chunk_mesh(
	atmo: AtmosphereGrid,
	face: int,
	chunk_u: int,
	chunk_v: int,
...
static func _march_cube_cached(
	...
	var cube_idx := 0
	for i in range(8):
		if corners[i] >= DENSITY_THRESHOLD:
			cube_idx |= (1 << i)

	if cube_idx == 0 or cube_idx == 255:
		return

	var edge_flags := _edge_table[cube_idx]
	if edge_flags == 0:
		return
	...
	for i in range(8):
		var cfu := fu + (step if (i == 1 or i == 2 or i == 5 or i == 6) else 0)
		var cfv := fv + (step if (i == 2 or i == 3 or i == 6 or i == 7) else 0)
		var calt := alt + (step if (i >= 4) else 0)
		positions[i] = atmo.get_cell_world_pos(face, cfu, cfv, calt, cloud_alt)
	...
	for e in range(12):
		if edge_flags & (1 << e):
			var a: int = edge_pairs[e][0]
			var b: int = edge_pairs[e][1]
			var t := _interp_factor(corners[a], corners[b])
			edge_verts[e] = positions[a].lerp(positions[b], t)

	var tri_list := _tri_table[cube_idx]
	var i := 0
	while i < tri_list.size() and tri_list[i] != -1:
		var v0 := edge_verts[tri_list[i]]
...
static func _interp_factor(val_a: float, val_b: float) -> float:
	if absf(val_a - val_b) < 0.0001:
		return 0.5
	return (DENSITY_THRESHOLD - val_a) / (val_b - val_a)


static func _init_edge_table() -> void:
	_edge_table = PackedInt32Array([
...
static func _init_tri_table() -> void:
	_tri_table = []
	_tri_table.resize(256)
	...
	for i in range(256):
		_tri_table[i] = PackedInt32Array(data[i])
