...
func update(world: Node, delta: float) -> void:
	...
	for _i in range(count):
		...
		var new_state := _utility_select(ecs, eid, species, ai, pos, hunger, health, energy)
...
func _rebuild_spatial_predators(ecs: EcsWorld) -> void:
	...
	for pid in predators.keys():
		...
		var cx := p.grid_x / SPATIAL_CELL
		var cy := p.grid_y / SPATIAL_CELL
		var key := cy * 1000 + cx
...
func _utility_select(ecs: EcsWorld, eid: int, species: ComFaunaSpecies, ai: ComAiState, pos: ComPosition, hunger: ComHunger, health: ComHealth, energy: ComEnergy) -> int:
	...
	if repro and species.age >= repro.maturity_age and hunger_urgency < 0.4 and energy_urgency < 0.3:
		u_mate = 0.6
...
func _sense_threat_spatial(ecs: EcsWorld, eid: int, pos: ComPosition) -> float:
	var positions := ecs.get_components("ComPosition")
	var threat := 0.0
	var cx := pos.grid_x / SPATIAL_CELL
	var cy := pos.grid_y / SPATIAL_CELL
	for dy in range(-1, 2):
		for dx in range(-1, 2):
...
func _move_toward_food(pos: ComPosition, species: ComFaunaSpecies) -> void:
	if grid == null:
		_move_random(pos, species)
		return
	...
	for dy in range(-1, 2):
		for dx in range(-1, 2):
			...
			var food_score: float = bdata.get("fertility", 0.3)
			if species.diet == DefEnums.DietType.HERBIVORE:
				food_score += bdata.get("tree_density", 0.0) * 0.5
