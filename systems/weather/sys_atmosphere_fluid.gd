...
func _apply_global_wind() -> void:
	...
	for face in range(AtmosphereGrid.NUM_FACES):
		for fv in range(AtmosphereGrid.FACE_RES):
			for fu in range(AtmosphereGrid.FACE_RES):
				var lat_frac := atmo_grid.get_latitude_fraction(face, fu, fv)
...
func _advection_step() -> void:
	var new_moisture := PackedFloat32Array()
	new_moisture.resize(AtmosphereGrid.TOTAL_CELLS)
	var new_temp := PackedFloat32Array()
	new_temp.resize(AtmosphereGrid.TOTAL_CELLS)
		...
		for fv in range(AtmosphereGrid.FACE_RES):
			for fu in range(AtmosphereGrid.FACE_RES):
				for alt in range(AtmosphereGrid.ALT_RES):
					...
					new_moisture[i] = _sample_trilinear(atmo_grid.moisture, face, src_fu, src_fv, src_alt)
					new_temp[i] = _sample_trilinear(atmo_grid.temperature, face, src_fu, src_fv, src_alt)

	for i in range(AtmosphereGrid.TOTAL_CELLS):
		atmo_grid.moisture[i] = new_moisture[i]
		atmo_grid.temperature[i] = new_temp[i]
